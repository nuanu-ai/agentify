...
const optionalGatewayHeaders = () => {
  ...
  if (!name || !value || !/^[A-Za-z0-9-]+$/.test(name))
    throw new Error(
      "Golden-corpus gateway header name/value must be supplied together",
    );
  if (
    ...
    throw new Error(
      "Golden-corpus gateway header conflicts with a required scan header",
    );
  return { [name]: value };
};
...
if (
  outputPath.startsWith(repositoryRoot + path.sep) &&
  !outputPath.startsWith(repositoryOutput)
)
  throw new Error(
    "Corpus output inside the repository must stay under ignored output/",
  );
...
if (selectedSites.some((site) => !authorizedIds.has(site.id)))
  throw new Error(
    "Every selected corpus target ID needs explicit authorization",
  );
for (const site of selectedSites) {
  ...
  if (
    ...
    throw new Error(
      `Manifest target ${site.id} is not a credential-free HTTP(S) URL`,
    );
...
const targetConcurrency = boundedInteger(
  "GOLDEN_CORPUS_TARGET_CONCURRENCY",
  2,
  1,
  10,
);
const pollIntervalMs = boundedInteger(
  "GOLDEN_CORPUS_POLL_INTERVAL_MS",
  500,
  100,
  5_000,
);
...
const submitAndPoll = async (site, runNumber) => {
  ...
  if (
    ...
    statusUrl.pathname !==
      `/api/v1/scans/${encodeURIComponent(body.scan_id)}/status`
  ) {
    ...
    ],
    {
      ...
      fetchStatus: async (scan) => {
        ...
        if (!statusResponse.ok)
          throw new Error(`status_${statusResponse.status}`);
  ...
  return {
    ...
    minimumCoverage:
      terminalRuns.length === 2
        ? Math.min(...terminalRuns.map((run) => run.coverage ?? 0))
        : null,
    mismatchedCheckIds,
...
await Promise.all(
  Array.from(
    { length: Math.min(targetConcurrency, selectedSites.length) },
    async () => {
...
const aggregateAgreement =
  targetResults.reduce((sum, target) => sum + target.agreement, 0) /
...
const evidence = {
  ...
  coverageQualifiedFraction: Number(
    (coverageQualifiedTargets / selectedSites.length).toFixed(4),
  ),
  allRunsUnder60Seconds: terminalRuns.every(
    (run) => run.terminalDurationMs < 60_000,
  ),
...
if (terminalRuns.length !== evidence.expectedRuns)
  failures.push(
    `${evidence.expectedRuns - terminalRuns.length} corpus runs were not terminal`,
  );
if (terminalRuns.some((run) => run.terminalStatus === "failed"))
  failures.push("one or more corpus runs ended failed");
if (!evidence.allRunsUnder60Seconds)
  failures.push("one or more corpus runs exceeded 60 seconds");
if (evidence.deterministicAgreement < 0.95)
  failures.push(
    `deterministic agreement ${evidence.deterministicAgreement} is below 0.95`,
  );

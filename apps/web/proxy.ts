...
export function proxy(request: NextRequest) {
  ...
  const rsc =
    request.nextUrl.searchParams.has("_rsc") || isReactServerComponentRequest(request.headers);

  if (eligibleMethod && !rsc && prefersMarkdown(request.headers.get("accept"))) {
    return new Response(method === "HEAD" ? null : getPublicPageMarkdown(pathname), {
      status: 200,
      headers: {
        ...responseHeaders,
  ...
  if (eligibleMethod && !rsc) {
    ...
    response.headers.set("Content-Language", responseHeaders["Content-Language"]);
...
export const config = {
  matcher: ["/", "/store", "/local", "/methodology", "/scanner", "/privacy", "/terms"],
};

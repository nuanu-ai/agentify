...
export function evaluateWorkerHeartbeat(
  ...
  const heartbeatFresh =
    Number.isFinite(heartbeatAt) && now.getTime() - heartbeatAt < 30_000;
  ...
  return {
    ...
    databaseConnected,
    ready:
      heartbeatFresh && queueConnected && databaseConnected && reportedReady,
  };
}

...
export async function POST(request: NextRequest) {
  ...
  const verified = await getVerifiedSession(request.cookies.get(REPORT_SESSION_COOKIE)?.value);
  if (!verified)
    return errorResponse(request, 404, "session_not_found", "A verified session is required.");
  ...
  if (parsed.data.type === "deletion") response.cookies.delete(REPORT_SESSION_COOKIE);
  return response;
}

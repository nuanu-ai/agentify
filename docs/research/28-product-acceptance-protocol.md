...
Auth work may begin once the candidate and mailbox are known. A missing funded buyer blocks only paid rows. Before the first paid row, operations records the buyer workflow, amount, network and asset, and where settlement and receipt will be observed. The private consumer caps each call at `0.01` test USDC and records no more than five calls in its local ledger. It refuses mainnet and self-pay. This ledger has no cross-process lock, so one operator serializes every paid command and no two ledger users run concurrently. The ordinary wrapper records an observed `settled:` line separately: its child exit also judges Bazaar listing, so a nonzero exit alone is not evidence that settlement failed.

The one-off recovery experiment ran `npm run recover-order -- item_818cef509a25432caf59eaa361129635 ord_491174f937484107acc52b7d2bba6ba5` with the worker stopped. The local ledger described the failed call only as `no_settlement_observed`; operations and the public status independently established `delivered_unpaid`. The original `PaymentRequired` had not been saved, so the command rebuilt it from the current TEST probe by removing its explanatory error and adding the known order identifier. The same order then settled once, produced one receipt and did not invoke or provision through the handler again. This proves the observed gateway recovery, not preservation of an original challenge or a reusable recovery command.

The main alias has a limit of three link requests per hour, and one address gets a new link only once a minute. A request made inside that minute is refused with the seconds left to wait, sends nothing and spends none of the three, so wait the minute out between requests rather than pressing again and treating the refusal as one of them. Use requests one and two for the independent browser sessions and reserve request three for reauthentication after the completed order is server-expired. Use one distinct, declared auth-edge alias for the expired unused link and any earlier session-expiry check. Test rate limiting in isolation; never edit a counter or global TTL.
...
| 2 | Request two links for the main alias before consuming either. Open each confirmation GET, reload and use Back/Forward, then confirm each by POST in a separate browser. Reuse one consumed link and sign out one session. **Browser** | GET/navigation does not consume a link, and the page names the address it signs in. Both links work independently once for the same merchant. A used link in a browser without a session explains recovery; in a signed-in browser it goes to that person's start and says nothing about the link. Signing out one session leaves the other valid. The third request remains unused until row 13. |
| 3 | For the auth-edge alias, operations expires one unused link. Open its GET and press its unchanged **Sign in as** button. Separately expire its valid session server-side and revisit a protected URL with browser storage intact. **Controlled TEST expiration + Browser** | GET still shows confirmation because it never checks or spends the token. POST is refused with `That link no longer works`, the once-and-an-hour explanation and **Ask for another link**. Expired session redirects to sign-in, never to a false empty state. |
| 4 | Exercise one session across the report and the cabinet for a fresh identity, then a returning sign-in without changing the known account. **Browser** | Fresh: full-report link → one-control page naming the address → report; **Open your cabinet** on the report reaches the cabinet in the same session with no second message and makes nothing; the merchant is made only by the cabinet's one control. Returning: a sign-in link with no destination opens the latest report for a person without a merchant and the cabinet for one with a merchant. Neither path duplicates or misattaches a merchant. |
| 5 | Run the existing isolated test that makes the gateway unavailable during merchant creation; do not disrupt shared TEST. **Isolated automated failure** | Person stays signed in, sees the fault and the same **Open my merchant cabinet** control, and pressing again creates one merchant without another email. Mark deployed UI unexercised unless a safe TEST injector exists. |

...
const parseTargetUrls = () => {
  ...
  return values.map((value) => {
    ...
    if (!["http:", "https:"].includes(url.protocol) || url.username || url.password)
      throw new Error("Every owned load target must be a credential-free HTTP(S) URL");
    return url;
  });
};
...
if (process.env.ALLOW_LOAD_TEST !== "true" || process.env.LOAD_TEST_ACK_OWNED_TARGET !== "yes") {
  throw new Error(
...
if (!["local", "staging", "production"].includes(environment))
  throw new Error("LOAD_TEST_ENVIRONMENT must be local, staging, or production");
const baseUrl = new URL(required("LOAD_TEST_BASE_URL"));
const workerBaseUrl = new URL(required("LOAD_TEST_WORKER_BASE_URL"));
if (![baseUrl, workerBaseUrl].every((url) => ["http:", "https:"].includes(url.protocol)))
  throw new Error("Load-test web and worker URLs must use HTTP(S)");
if (environment === "production" && process.env.ALLOW_PRODUCTION_LOAD_TEST !== "true")
...
if (strategy === "authorized_staging_bypass") {
  if (environment === "production")
    throw new Error("A staging rate-limit bypass must never be sent to production");
...
if (strategy === "owned_targets") {
  if (targets.length < 2)
    throw new Error("owned_targets strategy requires multiple explicitly owned targets");
  if (requests > 10)
    throw new Error(
...
if (!["store", "owner", "local"].includes(segment)) throw new Error("Invalid LOAD_TEST_SEGMENT");
const expectedMaxConcurrency = boundedInteger("LOAD_TEST_EXPECTED_MAX_CONCURRENCY", 10, 1, 10);
const pollIntervalMs = boundedInteger("LOAD_TEST_POLL_INTERVAL_MS", 500, 100, 5_000);
const terminalTimeoutMs = boundedInteger("LOAD_TEST_TERMINAL_TIMEOUT_MS", 75_000, 5_000, 180_000);
...
const workerConfiguredConcurrency = workerHealth?.checks?.configured_concurrency;
if (
  !workerReadiness.ok ||
  !Number.isInteger(workerConfiguredConcurrency) ||
  workerConfiguredConcurrency < 1
)
  throw new Error("Worker readiness must expose a positive integer checks.configured_concurrency");
...
const accepted = acceptance.accepted.filter((scan) => {
  try {
    ...
    return (
      ...
      statusUrl.pathname === `/api/v1/scans/${encodeURIComponent(scan.scanId)}/status`
    );
  } catch {
    return false;
  }
...
const terminal = polled.results.filter((result) => result.outcome === "terminal");
...
const failedTerminal = terminal.filter((result) => result.terminalStatus === "failed");
...
if (acceptance.acceptedResponses !== requests)
  failures.push(`${requests - acceptance.acceptedResponses} requests were not accepted`);
...
if (lostIds) failures.push(`${lostIds} accepted scan IDs did not reach terminal state`);
if (failedTerminal.length) failures.push(`${failedTerminal.length} scans ended failed`);
if (acceptance.apiP95Ms >= 500)
  failures.push(`accepted API p95 ${acceptance.apiP95Ms.toFixed(1)}ms exceeds 500ms`);
if (queueP95Ms >= 5_000) failures.push(`queue p95 ${queueP95Ms.toFixed(1)}ms exceeds 5000ms`);
if (terminalP95Ms >= 60_000)
  failures.push(`terminal scan p95 ${terminalP95Ms.toFixed(1)}ms exceeds 60000ms`);

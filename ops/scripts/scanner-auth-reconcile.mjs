...
if (!sourcePath || !targetPath || !reportPath) {
  console.error(
    "Usage: scanner-auth-reconcile.mjs SOURCE_AUTH_NDJSON TARGET_LEADS_NDJSON PROTECTED_REPORT [--private-baseline] [NEW_AUTH_NDJSON]",
  );
  process.exit(1);
}
if (
  newAuthPaths.length > 1 ||
  (privateBaseline && extras.filter((arg) => arg === "--private-baseline").length !== 1)
) {
...
for (const user of source) {
  if (
    ...
    typeof user.confirmed !== "boolean"
  ) {
    throw new Error("Source Auth inventory has an invalid or duplicate user.");
  }
  users.set(user.id, user);
}
const freshVerifiedEmails = new Set(
  newAuth.filter((u) => u.email_verified === true).map((u) => u.email?.toLowerCase()),
);
const linkedCounts = new Map();
const foreignLeadLinks = [];
for (const lead of links) {
  if (!lead.id || !lead.supabase_user_id) continue;
  if (!users.has(lead.supabase_user_id))
    foreignLeadLinks.push({ lead_id: lead.id, auth_id: lead.supabase_user_id });
  linkedCounts.set(lead.supabase_user_id, (linkedCounts.get(lead.supabase_user_id) ?? 0) + 1);
}
const changedLinks = privateBaseline
  ? []
  : source.filter((u) => Number(u.linked_leads) !== (linkedCounts.get(u.id) ?? 0));
const confirmedUnlinked = source.filter(
  (u) =>
    u.confirmed &&
    (privateBaseline ? (linkedCounts.get(u.id) ?? 0) : Number(u.linked_leads)) === 0 &&
    !freshVerifiedEmails.has(u.email.toLowerCase()),
);
...
const report = {
  ...
  changed_source_links: changedLinks.map((u) => ({
    auth_id: u.id,
    source_count: u.linked_leads,
    target_count: linkedCounts.get(u.id) ?? 0,
  })),
  confirmed_unlinked_needing_fresh_verification: confirmedUnlinked.map((u) => ({
    auth_id: u.id,
    email: u.email,
  })),
...
console.log(
  `Identity custody: source=${source.length}, linked=${report.target_linked_leads}, foreign=${foreignLeadLinks.length}, changed=${changedLinks.length}, confirmed-unlinked=${confirmedUnlinked.length}, pending=${pending.length}.`,
);
if (foreignLeadLinks.length || changedLinks.length || confirmedUnlinked.length)
  process.exitCode = 2;

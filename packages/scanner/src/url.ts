...
const SECRET_KEY = /(?:^|[_-])(?:token|key|secret|password|signature|auth)(?:$|[_-])/i;
...
const looksLikeObfuscatedIpv4 = (hostname: string): boolean => {
  ...
  return (
    ...
    parts.some((part) => part.startsWith("0x") || (part.length > 1 && part.startsWith("0")))
  );
};
...
export const canonicalizeTarget = (input: string): URL => {
  ...
  if (keys.some((key) => SECRET_KEY.test(key))) throw new UrlPolicyError("secret_query_blocked");
  for (const key of keys) if (TRACKING_KEY.test(key)) url.searchParams.delete(key);
...
export const sameOrigin = (left: URL, right: URL): boolean => left.origin === right.origin;
